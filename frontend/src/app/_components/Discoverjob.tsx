...
function VacancyCard() {
  ...
  const handleApply = (post: PostType) => {
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-8">
          {sortedPosts.map((post, index) => {
            ...
            const postDate = new Date(post.postedAt).toLocaleDateString(
              "en-GB",
              {
                day: "numeric",
                month: "short",
            ...
            return (
              <div
                key={index}
                className="flex flex-col justify-between bg-white dark:bg-zinc-800 shadow-md rounded-2xl p-5 h-full transition-all duration-300 hover:shadow-xl"
              >
